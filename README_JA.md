...
3. [http://localhost:3000](http://localhost:3000) を開きます。フロントエンドはローカル Rust サービス `http://127.0.0.1:8788` に接続します。
   ローカル開発では `8788` ではなくフロントエンド開発用 URL を開いてください。`8788` は主にローカル API 用であり、古い `dist` バンドルを誤って配信しないように `pnpm dev` ではバックエンドの静的フロントエンド配信をデフォルトで無効化します。
